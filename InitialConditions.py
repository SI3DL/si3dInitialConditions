...
if DeltaZ == 'constant':
    if TempProf == 'constant':
        [T,z] = initCond4si3d(LakeName,SimStartDate,DeltaZ,TempProf,PathSave, NTracers,z_Tr=z_Tr,conc_Tr=conc_Tr,H=H,dz=dz,Tc=Tc)
    elif TempProf == 'variable':
        [T,z] = initCond4si3d(LakeName,SimStartDate,DeltaZ,TempProf,PathSave, NTracers,z_Tr=z_Tr,conc_Tr=conc_Tr,H=H,dz=dz, z_CTD=z_CTD,T_CTD=T_CTD)
elif DeltaZ == 'variable':
    if TempProf == 'constant':
        [T,z] = initCond4si3d(LakeName,SimStartDate,DeltaZ,TempProf,PathSave, NTracers,z_Tr=z_Tr,conc_Tr=conc_Tr, H=H,Tc=Tc,spacingMethod=spacingMethod, dz0s=dz0s,dz0b=dz0b,dzxs=dzxs,dzxb=dzxb,n=n,Hn=Hn,dzc=dzc)
    elif TempProf == 'variable':
        [T,z] = initCond4si3d(LakeName,SimStartDate,DeltaZ,TempProf,PathSave, NTracers,z_Tr=z_Tr,conc_Tr=conc_Tr, H=H,z_CTD=z_CTD,T_CTD=T_CTD, spacingMethod=spacingMethod,dz0s=dz0s,dz0b=dz0b,dzxs=dzxs,dzxb=dzxb,n=n,Hn=Hn,dzc=dzc)

fidinp = open('si3d_inp.txt',"r+")
lines = fidinp.readlines()

if DeltaZ == 'constant':
    lines[18] = 'idz          !       '+str(dz)+'.0          ! Cell size (m) in vertical\n'
    ...
    lines[18] = 'idz          !       '+str(dzc)+'.0          ! Cell size (m) in vertical\n'
